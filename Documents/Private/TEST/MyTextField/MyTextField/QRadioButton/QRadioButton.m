...
- (id)initWithDelegate:(id)delegate groupId:(NSString*)groupId {
    ...
    if (self) {
        ...
        [self setImage:[UIImage imageNamed:@"t1_radio_unchecked.png"] forState:UIControlStateNormal];
        [self setImage:[UIImage imageNamed:@"t1_radio_checked.png"] forState:UIControlStateSelected];
...
- (void)addToGroup {
    if(!_groupRadioDic){
        _groupRadioDic = [[NSMutableDictionary alloc] init];//[[NSMutableDictionary dictionary] retain];
    }
...
- (void)setChecked:(BOOL)checked {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(didSelectedRadioButton:groupId: select:)]) {
        [_delegate didSelectedRadioButton:self groupId:_groupId select:self.selected];
    }
}
